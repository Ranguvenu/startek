...
$userext = optional_param('userext', 0, PARAM_INT); // test session works properly
...
//changed the testsession var to userext
//eAbyas change for vulnerability
if ($userext) {
    if ($userext == $USER->id) {
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    // Vulnerability checking starts.
    if (!empty($frm->hashpassword)) {
        $original_password = base64_decode($frm->hashpassword);
        $orgtohash = hash('sha256', $original_password);

        $original_username = base64_decode($frm->hashusername);
        $orgtohashusername = hash('sha256', $original_username);

        if ($orgtohash === $frm->password) {
            $frm->password = $original_password;
        } else {
            $errormsg = get_string("invalidlogin");
            $errorcode = 3;
        }
        if ($orgtohashusername === $frm->username) {
            $frm->username = $original_username;
        } else {
            $errormsg = get_string("invalidlogin");
            $errorcode = 3;
        }
    } // Vulnerability checking ends.
    ...
    /// Currently supported only for ldap-authentication module
        ...
        redirect(new moodle_url(get_login_url(), array('userext'=>$USER->id)));
