...
class enrol_certification_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_certification_enrol($instance, false) !== true) {
        ...
        $icons = array();
            ...
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_certification'), 'enrol_certification');
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_certification_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/certification:config', $context)) {
		 ...
		 if ($DB->record_exists('enrol', array('courseid'=>$courseid, 'enrol'=>'certification'))) {
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/certification/locallib.php");

        $enrolstatus = $this->can_certification_enrol($instance);

        if (true === $enrolstatus) {
            // This user can self enrol using this instance.
            $form = new enrol_certification_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_certification($instance, $data);
                }
            }
        } else {
     ...
     */
    public function can_certification_enrol(stdClass $instance, $checkuserenrolment = true) {
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_certification_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_certification');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_certification_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/certification:unenrol", $context)) {
