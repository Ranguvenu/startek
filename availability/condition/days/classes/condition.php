...
class condition extends \core_availability\condition {
    ...
    protected function get_reference_date() {
        ...
        if (empty($this->custom_course))
        $custom_course = $DB->get_record('course', array('id'=>$COURSE->id));
        else
        $custom_course = $DB->get_record('course', array('id'=>$this->custom_course));
        ...
        if (empty($config->referencedate) || $config->referencedate == 'coursestartdate') {
            ...
            $referencedate = $custom_course->startdate;
        } else {
            ...
            $sql = '
                ...
                GROUP BY
                    ue.userid
            ';
            if ($lowest = $DB->get_record_sql($sql, array($custom_course->id, $USER->id))) {
                if ($lowest->minenroltime)
                $referencedate = $lowest->minenroltime;
                else
                $referencedate = $lowest->timecreated;
            } else {
