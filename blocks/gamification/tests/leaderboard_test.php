...
use block_gamification\local\leaderboard\anonymisable_leaderboard;
use block_gamification\local\leaderboard\course_user_leaderboard;
use block_gamification\local\leaderboard\neighboured_leaderboard;
use block_gamification\local\leaderboard\relative_ranker;
use block_gamification\local\leaderboard\null_ranker;
use block_gamification\local\sql\limit;
use block_gamification\local\gamification\full_anonymiser;
...
class leaderboard_test extends base_testcase {
     ...
     *
     * @covers \block_gamification\local\leaderboard\course_user_leaderboard
     */
    public function test_basic_leaderboard() {
        ...
        $this->assert_ranking($ranking, $egamificationected);
     ...
     *
     * @covers \block_gamification\local\leaderboard\anonymisable_leaderboard
     */
     ...
     *
     * @covers \block_gamification\local\leaderboard\neighboured_leaderboard
     */
    ...
    public function test_neighboured_leaderboard_top_fallback() {
        ...
        // otherwise we will get unegamificationected results as it's not how the leaderboard
        // should be used.
     ...
     *
     * @covers \block_gamification\local\leaderboard\relative_ranker
     */
    public function test_relative_ranker() {
        ...
        $egamificationected = [[$u3, 10], [$u2, 0], [$u1, -20]];
        $this->assert_ranking($lb->get_ranking(new limit(0, 0)), $egamificationected);
        ...
        $egamificationected = [[$u3, 0], [$u2, -10], [$u1, -30]];
        $this->assert_ranking($lb->get_ranking(new limit(0, 0)), $egamificationected);
    }
    ...
    public function test_null_ranker() {
        ...
        $this->assertEquals(0, $lb->get_rank($u1->id)->get_rank());
        $this->assertEquals(0, $lb->get_rank($u3->id)->get_rank());
        $egamificationected = [[$u3, 0], [$u2, 0], [$u1, 0]];
        $this->assert_ranking($lb->get_ranking(new limit(0, 0)), $egamificationected);
    }

    protected function assert_ranking($ranking, array $egamificationected) {
        $i = 0;
        foreach ($ranking as $rank) {
            $this->assertEquals($egamificationected[$i][0]->id, $rank->get_state()->get_id(), $i);
            $this->assertEquals($egamificationected[$i][1], $rank->get_rank(), $i);
            $i++;
        }
        $this->assertEquals(count($egamificationected), $i);
    }
