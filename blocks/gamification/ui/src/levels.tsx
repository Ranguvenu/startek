...
const updateLevelPoints = (state: State): State => {
  if (!state.algo.enabled) {
    return {
      ...
      levels: state.levels.reduce<State['levels']>((carry, level, i) => {
        return carry.concat([
          { ...level, gamificationrequired: Math.max(level.gamificationrequired, getMinimumPointsForLevel(carry.concat([level]), level)) },
        ]);
      }, []),
    };
  }
  return {
    ...
    levels: state.levels.map((level) => {
      return { ...level, gamificationrequired: computeRequiredPoints(level.level, state.algo.base, state.algo.coef) };
    }),
  };
};
  ...
  switch (action) {
    ...
    case 'levelPointsChange':
      ...
      if (isNaN(payload.points) || payload.points <= 2 || payload.points >= Infinity) {
        return state;
      } else if (payload.points <= getPreviousLevel(state.levels, payload.level).gamificationrequired) {
        return state;
      }
      return markPendingSave(
        updateLevelPoints({
          ...
          levels: state.levels.map((level) => {
            ...
            return { ...level, gamificationrequired: payload.points };
          }),
        })
      );
    case 'nbLevelsChange':
...
const App = ({ courseId, levelsInfo }: { courseId: number; levelsInfo: LevelsInfo }) => {
  ...
  const mutation = useMutation(
    () => {
      ...
      let method = courseId ? 'block_gamification_set_levels_info' : 'block_gamification_set_default_levels_info';
  ...
  return (
    <div>
      <div className="gamification-flex gamification-flex-col gamification-flex-wrap">
        <div className="gamification-mb-4">
          <Menu
            algo={state.algo}
        ...
        <div className="gamification-flex-1">
...
const MenuItem: React.FC<{ label?: React.ReactNode; className?: string }> = ({ label, className = '', children }) => {
  return (
    <div className={`gamification-mb-2 gamification-mr-2 ${className}`}>
...
const Menu: React.FC<{
  ...
  return (
    <div className="gamification-flex gamification-flex-col md:gamification-flex-row">
      <div className="gamification-flex gamification-flex-col gamification-flex-wrap gamification-order-2 md:gamification-order-none md:gamification-flex-row">
        <MenuItem
          label={
            <label htmlFor="bgamificationViewAs" className="gamification-m-0">
              <Str id="viewas" />
            </label>
          }
        >
          <select
            id="bgamificationViewAs"
            value={view}
            onChange={(e) => onViewChange(e.target.value as 'list' | 'grid')}
            className="form-control gamification-m-0 gamification-max-w-xs"
          >
        ...
        <MenuItem
          label={
            <label htmlFor="bgamificationNbLevels" className="gamification-m-0">
              <Str id="levelcount" />
            </label>
          }
        >
          <NumberInput
            id="bgamificationNbLevels"
            min={2}
            max={99}
            size={4}
        ...
        <MenuItem
          label={
            <label htmlFor="bgamificationAlgoEnabled" className="gamification-m-0">
              <Str id="setpoints" />
            </label>
          }
        >
          <select
            id="bgamificationAlgoEnabled"
        ...
        {algo.enabled ? (
          <>
            <MenuItem
              label={
                <label htmlFor="bgamificationAlgoPts" className="gamification-m-0">
                  <Str id="basegamification" />
                </label>
              }
            >
              <NumberInput
                id="bgamificationAlgoPts"
                min={1}
                size={4}
            ...
            <MenuItem
              label={
                <label htmlFor="bgamificationAlgoCoef" className="gamification-m-0">
                  <Str id="coefgamification" />
                </label>
              }
            >
              <NumberInput
                id="bgamificationAlgoCoef"
                min={1}
                max={5}
                size={4}
                step={0.1}
      ...
      <div className="gamification-flex-1 gamification-flex md:gamification-justify-end">
        <MenuItem className="md:gamification-hidden">
          <SaveButton statePosition="after" onClick={onSave} mutation={mutation} disabled={!canSave} />
        </MenuItem>
        <MenuItem className="gamification-hidden md:gamification-block">
