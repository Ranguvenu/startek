YUI.add('moodle-block_gamification-filters', function (Y, NAME) {
 ...
 * Filters constants.
 *
 * @module     moodle-block_gamification-filters
 * @package    block_gamification
 * @copyright  2015 Frédéric Massart
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @main       moodle-block_gamification-filters
 */
...
var CSS = {
    ...
    PREFIX: 'block_gamification-filters',
    RULE: 'rule',
    RULES: 'rule-rules',
};
var SELECTORS = {
    ...
    CONTAINER: '.block-gamification-filters',
...
Y.namespace('M.block_gamification').Filters = Y.extend(FILTERS, Y.Base, {
    ...
    initializer: function() {
        ...
        this.filterDnD = Y.namespace('M.block_gamification.Filters.DnD').init({
    ...
    deleteFilter: function(e) {
        ...
        if (firstRule && this.countChildrenRulesInRule(firstRule) > 0) {
            var confirm = new M.core.confirm({
                title: M.util.get_string('deleterule', 'block_gamification'),
    ...
    deleteRule: function(e) {
        ...
        if (this.countChildrenRulesInRule(rule) > 0) {
            var confirm = new M.core.confirm({
                title: M.util.get_string('deletecondition', 'block_gamification'),
                question: M.util.get_string('areyousure', 'core'),
            });
            confirm.on('complete-yes', deleteOperation, this);
            confirm.show();
    ...
    prepareRuleDialog: function() {
        ...
        this.rulepicker = Y.namespace('M.block_gamification.RulePicker').init({
            rules: rules
        });
    ...
    setFilterRulesDnD: function(filterNode) {
        ...
        this.rulesDnD[filterNode.generateID()] = Y.namespace('M.block_gamification.Filters.DnD').init({
...
Y.namespace('M.block_gamification.Filters').init = function(config) {
 ...
 * Filters Drag and Drop.
 *
 * @namespace Y.M.block_gamification.Filters.DnD
 * @class DnD
 * @constructor
 */
var DND = function() {
...
Y.namespace('M.block_gamification.Filters').DnD = Y.extend(DND, M.core.dragdrop, {
    ...
    NAME: 'block_gamification-filters-dnd',
    ATTRS: {
...
Y.namespace('M.block_gamification.Filters.DnD').init = function(config) {
    ...
    "requires": [
        ...
        "moodle-block_gamification-rulepicker"
    ]
});
