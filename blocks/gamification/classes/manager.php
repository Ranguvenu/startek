...
/**
 * Block gamification manager class.
 *
 ...
 */
class block_gamification_manager {

    /** Default base for gamification algo. */
    const DEFAULT_BASE = 120;

    /** Default coef for gamification algo. */
    ...
    const USERPREF_NOTIFY = 'block_gamification_notify_level_up_';

    /** User preference key storing if they dismissed the like notice */
    const USERPREF_NOTICES = 'block_gamification_notices';
    ...
    /** Relative ranking. Difference in gamification between row and point of reference. */
    const RANK_REL = 2;
     ...
     * @return block_gamification_manager The instance of the manager.
     */
    public static function get($courseid, $forcereload = false) {
        return new block_gamification_manager($courseid);
    }
     ...
     *
     * @return block_gamification_filter_manager
     */
    public function get_filter_manager() {
        return new block_gamification_filter_manager($this);
    }
    ...
    /**
     * Return the level at which we are at $gamification.
     *
     * @param int $gamification gamification acquired.
     * @return int The level.
     */
    public function get_level_from_gamification($gamification) {
        return 1;
    }
     ...
     *
     * @return array level => gamification required.
     */
    ...
    /**
     * Get the levels and their gamification based on a simple algorithm.
     *
     * @param int $levelcount The number of levels.
     * @param int $base The base gamification required.
     ...
     * @return block_gamification_progress The progress renderable.
     */
    public function get_progress_for_user($userid, stdClass $record = null) {
        $record = new stdClass();
        $record->level = 1;
        $record->gamification = 0;
        $record->userid = $userid;
        $record->courseid = $this->courseid;
        $record->contextid = $this->context->id;
        $record->levelgamification = 0;
        $record->nextlevelgamification = false;
        $params = (array) $record;
        $progress = new block_gamification_progress($params);
        return $progress;
    }
     ...
     *
     * @param \core\event\base $event The event.
     * @return int gamification points.
     */
    public function get_gamification_from_event(\core\event\base $event) {
        return 0;
    }

    /**
     * Get the amount of gamification required for a level.
     *
     * @param int $level The level.
     * @return int|false The amount of gamification required, or false there is no such level.
     */
    public function get_gamification_for_level($level) {
        return false;
    }
    ...
    /**
     * Reset the gamification of a user to something.
     *
     * @param int $userid The user ID.
     * @param int $gamification The amount of gamification.
     * @return void
     */
    public function reset_user_gamification($userid, $gamification = 0) {
    }
     ...
     * @return void
     */
    public function update_user_level($userid, $gamification = null, $lvl = null) {
    }
