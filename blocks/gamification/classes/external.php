...
use block_gamification\local\utils\external_utils;
use block_gamification\local\gamification\algo_levels_info;
use context_course;
use context_system;
use core_text;
use external_api;
...
class external extends external_api {
    ...
    public static function set_default_levels_info_parameters() {
        return new external_function_parameters([
            'levels' => new external_multiple_structure(new external_single_structure([
                ...
                'gamificationrequired' => new external_value(PARAM_INT),
    ...
    public static function set_levels_info($courseid, $levels, $algo) {
        ...
        if (count($levels) < 2 || count($levels) > 99) {
            ...
            $levelsdata = array_reduce(array_keys($levels), function($carry, $key) use ($levels, &$lastpts) {
                ...
                if ($lastpts === null) {
                    ...
                    $gamification = min(max($lastpts + 1, $level['gamificationrequired']), PHP_INT_MAX);
                }

                $carry['gamification'][$levelnb] = $gamification;
                if (!empty($level['name'])) {
                ...
                $lastpts = $gamification;
                return $carry;
            }, ['gamification' => [], 'name' => [], 'desc' => []]);
            ...
            $levelsinfo = new algo_levels_info([
                'gamification' => $levelsdata['gamification'],
        ...
        if ($store instanceof \block_gamification\local\gamification\course_user_state_store) {
