 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_gamification_rule_event extends block_gamification_rule_property {
    ...
    public function __construct($eventname = '') {
        ...
        $this->eventlister = \block_gamification\di::get('rule_event_lister');
    }
    ...
    public function get_description() {
        ...
        if ($infos !== false) {
            ...
            $name = get_string('colon', 'block_gamification', (object) [
                'a' => $displayname,
                'b' => $infos['name']
            ]);
        } else {
            $name = get_string('unknowneventa', 'block_gamification', $this->value);
        }

        return get_string('ruleeventdesc', 'block_gamification', ['eventname' => $name]);
    }
    ...
    public static function get_event_infos($class) {
        return \block_gamification\local\rule\event_lister::get_event_infos($class);
    }
    ...
    public static function get_events_list() {
        debugging('The method block_gamification_rule_event::get_events_list() is deprecated.', DEBUG_DEVELOPER);
        return [];
    }
    ...
    public function get_form($basename) {
        ...
        if (!empty($this->value) && !$this->value_in_list($this->value, $eventslist)) {
            $eventslist[] = [get_string('other') => [$this->value => get_string('unknowneventa', 'block_gamification', $this->value)]];
        }
        ...
        $o .= get_string('eventis', 'block_gamification', '');
        $o .= html_writer::end_div();

        $o .= html_writer::div($modules, 'gamification-min-w-px gamification-max-w-[80%]');
        $o .= html_writer::end_div();
