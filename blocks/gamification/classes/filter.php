 ...
 * Filter class.
 *
 * The filter only works with block_gamification_rule_property, or rulesets containing them.
 *
 * @package    block_gamification
 * @copyright  2014 Frédéric Massart - FMCorz.net
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_gamification_filter implements renderable {
    ...
    public function delete() {
        ...
        $DB->delete_records('block_gamification_filters', array('id' => $this->id));
    }
     ...
     *
     * @return block_gamification_rule
     */
     ...
     * @return block_gamification_filter The filter.
     */
    ...
    protected function load_rule() {
        ...
        $this->rule = block_gamification_rule::create($ruledata);
        ...
        if ($this->rule === false) {
            $this->set_rule(new \block_gamification_ruleset());
        }
    }
    ...
    public function save() {
        ...
        $record = $this->export();
        if (!$this->id) {
            $this->id = $DB->insert_record('block_gamification_filters', $record);
        } else {
            $record->id = $this->id;
            $DB->update_record('block_gamification_filters', $record);
        }
    }
     ...
     *
     * @param block_gamification_rule $rule
     */
    public function set_rule(block_gamification_rule $rule) {
        $this->rule = $rule;
    ...
    protected static function validate_ruledata($ruledata) {
        ...
        foreach ($ruledata as $key => $value) {
            ...
            if ($key == '_class') {
                ...
                $valid = $reflexion->isSubclassOf('block_gamification_rule');
            } else if (is_array($value)) {
                $valid = block_gamification_rule::validate_data($value);
            }
        }
        return $valid;
    }
