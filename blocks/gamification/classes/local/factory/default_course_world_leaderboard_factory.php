...
use block_gamification\local\leaderboard\leaderboard;
use block_gamification\local\leaderboard\neighboured_leaderboard;
use block_gamification\local\leaderboard\null_ranker;
use block_gamification\local\leaderboard\ranker;
...
use block_gamification\local\gamification\state_anonymiser;
    ...
    protected function get_columns_with_config(course_world $world, config $config) {
        ...
        if ($config->get('rankmode') != course_world_config::RANK_OFF) {
            if ($config->get('rankmode') == course_world_config::RANK_REL) {
                $columns['level'] = new lang_string('level', 'block_gamification');
                $columns['rank'] = new lang_string('difference', 'block_gamification');
            } else {
                $columns['rank'] = new lang_string('rank', 'block_gamification');
                $columns['level'] = new lang_string('level', 'block_gamification');
            }
        } else {
            $columns['level'] = new lang_string('level', 'block_gamification');
        }
        $columns['fullname'] = new lang_string('participant', 'block_gamification');

        $additionalcols = explode(',', $config->get('laddercols'));
        if (in_array('gamification', $additionalcols)) {
            $columns['gamification'] = new lang_string('total', 'block_gamification');
        }
        if (in_array('progress', $additionalcols)) {
            $columns['progress'] = new lang_string('progress', 'block_gamification');
        }
