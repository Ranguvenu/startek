 ...
 * A repository for getting activity based on course logs.
 *
 * This is hardcoded based on the block_gamification\local\logger\course_user_event_collection_logger.
 *
 * @package    block_gamification
 * @copyright  2017 Frédéric Massart
 * @author     Frédéric Massart <fred@branchup.tech>
...
class course_log_recent_activity_repository implements user_recent_activity_repository {
    ...
    protected $table = 'block_gamification_log';
    ...
    public function get_user_recent_activity($userid, $count = 0) {
        $results = $this->db->get_records_select($this->table, 'courseid = :courseid AND userid = :userid AND gamification > 0', [
        ...
        return array_map(function($row) {
            ...
            if (class_exists($class) && is_subclass_of($class, 'core\event\base')) {
                ...
                $desc = new lang_string('somethinghappened', 'block_gamification');
            }

            return new gamification_activity(
