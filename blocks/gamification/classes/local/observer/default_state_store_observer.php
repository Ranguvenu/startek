...
use block_gamification\local\gamification\state_store;
...
class default_state_store_observer implements level_up_state_store_observer, points_increased_state_store_observer {
    ...
    public function leveled_up(state_store $store, $id, level $beforelevel, level $afterlevel) {
        ...
        for ($i = $lowestlevel; $i <= $highestlevel; $i++) {
            ...
            $lupevent = \block_gamification\event\user_leveledup::create($params);
            $lupevent->trigger();
    ...
    public function points_increased(state_store $store, $id, $pointsamount) {
        ...
        $hooks = get_plugin_list_with_function('block', 'gamification_points_increased');
        foreach ($hooks as $plugin => $fullfunctionname) {
            try {
                component_callback($plugin, 'gamification_points_increased', [$this->context, $id, $pointsamount]);
            } catch (\Exception $e) {
                debugging("Error while calling $plugin's gamification_points_increased callback: " . $e->getMessage(), DEBUG_DEVELOPER);
            }
        }
    }
