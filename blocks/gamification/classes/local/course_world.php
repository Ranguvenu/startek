...
use block_gamification\local\factory\badge_url_resolver_course_world_factory;
...
class course_world implements world {
    ...
    public function __construct(config $config, moodle_database $db, $courseid,
        ...
        $this->perms = new \block_gamification\local\permission\context_permissions($this->context);
    }
    ...
    public function get_collection_strategy() {
        if (!$this->strategy) {
            $this->strategy = new \block_gamification\local\strategy\course_world_collection_strategy(
                $this->get_context(),
                $this->get_config(),
                $this->get_store(),
     ...
     * Get filter manager.
     *
     * @return gamification\course_filter_manager
     */
    public function get_filter_manager() {
        if (!$this->filtermanager) {
            $this->filtermanager = new \block_gamification\local\gamification\course_filter_manager($this->db, $this->courseid);
    ...
    public function get_levels_info() {
        if (!$this->levelsinfo) {
            ...
            if (!$data) {
                $this->levelsinfo = \block_gamification\local\gamification\algo_levels_info::make_from_defaults($resolver);
            } else {
                $this->levelsinfo = new \block_gamification\local\gamification\algo_levels_info($data, $resolver);
            }
        }
    ...
    public function get_level_up_notification_service() {
        ...
        return new \block_gamification\local\notification\course_level_up_notification_service($this->courseid);
    }

    /**
     * Get the level up state store observer.
     *
     * @return \block_gamification\local\observer\level_up_state_store_observer
     */
     ...
     *
     * @return \block_gamification\local\observer\points_increased_state_store_observer
     */
    ...
    protected function get_state_store_observer() {
        if (!$this->statestoreobserver) {
            $this->statestoreobserver = new \block_gamification\local\observer\default_state_store_observer($this->context, $this->config,
                $this->get_level_up_notification_service());
        }
    ...
    public function get_store() {
        if (!$this->store) {
            $this->store = new \block_gamification\local\gamification\course_user_state_store(
                $this->db,
    ...
    private function get_collection_logger() {
        if (!$this->logger) {
            $this->logger = new \block_gamification\local\logger\course_user_event_collection_logger($this->db, $this->courseid);
        }
        return $this->logger;
    }
    ...
    public function get_user_recent_activity_repository() {
        return new \block_gamification\local\activity\course_log_recent_activity_repository($this->db, $this->courseid);
    }
