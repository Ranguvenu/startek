...
use Traversable;
use block_gamification\local\iterator\map_iterator;
...
class relative_ranker implements ranker {
    ...
    public function rank_state(state $state) {
        $base = $this->relativestate ? $this->relativestate->get_gamification() : $state->get_gamification();
        return new state_rank($state->get_gamification() - $base, $state);
    }
    ...
    public function rank_states($states) {
        $base = $this->relativestate ? $this->relativestate->get_gamification() : null;
        return new map_iterator($states, function($state) use (&$base) {
            if ($base === null) {
                $base = $state->get_gamification();
            }
            return new state_rank($state->get_gamification() - $base, $state);
        });
    }
