...
abstract class page_controller extends course_route_controller {
    ...
    protected function content() {
       ...
       /* if ($this->world->get_access_permissions()->can_manage()) {
            ...
            if (!$isforwholesite && $requestedcourseid == SITEID) {
                ...
                echo $output->notification_without_close(get_string('errorcontextcoursemismatchpercourse', 'block_gamification'),
                ...
                echo $output->notification_without_close(get_string('errorcontextcoursemismatchforwholesite', 'block_gamification',
        ...
        echo html_writer::start_div('gamification-w-full gamification-flex gamification-flex-col lg:gamification-flex-row gamification-gap-6');
        if ($this->has_sub_navigation()) {
            $this->page_sub_navigation();
        }
        echo html_writer::start_div('gamification-flex-1 gamification-w-full gamification-min-w-px');
        // $this->page_notices();
        $this->page_content();
    ...
    protected function page_sub_navigation() {
        $output = $this->get_renderer();
        echo html_writer::start_div('gamification-w-full lg:gamification-w-36 gamification-max-w-full');
