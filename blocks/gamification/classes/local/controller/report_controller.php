...
class report_controller extends page_controller {
    ...
    protected function permissions_checks() {
        ...
        if (!($accessperms instanceof \block_gamification\local\permission\access_report_permissions)) {
    ...
    protected function pre_content() {
        ...
        if ($this->get_param('resetdata') && confirm_sesskey()) {
            if ($this->get_param('confirm')) {
                ...
                if ($this->get_groupid()) {
                    ...
                    if ($store instanceof \block_gamification\local\gamification\course_state_store) {
        ...
        if ($action === 'edit' && !empty($userid)) {
            ...
            if ($data = $form->get_data()) {
                $this->world->get_store()->set($userid, $data->gamification);
    ...
    protected function get_page_html_head_title() {
        return get_string('coursereport', 'block_gamification');
    }
    ...
    protected function get_form($userid) {
        if (!$this->form) {
            ...
            $form = new \block_gamification\form\user_gamification($this->pageurl->out(false));
            $form->set_data(['userid' => $userid, 'level' => $state->get_level()->get_level(), 'gamification' => $state->get_gamification()]);
            $this->form = $form;
        }
        return $form;
    }
    ...
    protected function get_table() {
        if (!$this->table) {
            $this->table = new \block_gamification\output\report_table(
                \block_gamification\di::get('db'),
                $this->world,
                $this->get_renderer(),
    ...
    protected function get_bottom_action_buttons() {
        ...
        $strreset = null;
        if (empty($groupid)) {
            $strreset = get_string('resetcoursedata', 'block_gamification');
        } else if ($this->world->get_store() instanceof \block_gamification\local\gamification\course_state_store) {
            $strreset = get_string('resetgroupdata', 'block_gamification');
        }
    ...
    protected function page_content() {
        ...
        if ($canmanage && $this->get_param('resetdata')) {
            echo $this->get_renderer()->confirm(
                empty($groupid) ? get_string('reallyresetdata', 'block_gamification') : get_string('reallyresetgroupdata', 'block_gamification'),
        ...
        if ($canmanage && $this->get_param('delete')) {
            echo $this->get_renderer()->confirm(
                markdown_to_html(get_string('reallydeleteuserstateandlogs', 'block_gamification')),
        ...
        echo html_writer::start_div('gamification-cancel-overflow'); // Else dropdown menu is cropped on some versions.
