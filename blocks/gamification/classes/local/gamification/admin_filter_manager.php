...
class admin_filter_manager {
    ...
    public function get_all_filters() {
        ...
        //     return $this->get_default_filters(\block_gamification_filter::CATEGORY_EVENTS);
        // } //comment by revathi

        $results = $this->db->get_recordset('block_gamification_filters', ['courseid' => 0], 'sortorder ASC, id ASC');
        $filters = [];
        foreach ($results as $key => $filter) {
            $filters[$filter->id] = \block_gamification_filter::load_from_data($filter);
        }
        $results->close();
        return $filters;
    }
     ...
     * @return block_gamification_filter[]
     */
    protected function get_default_filters($category = \block_gamification_filter::CATEGORY_EVENTS) {
        if ($category != \block_gamification_filter::CATEGORY_EVENTS) {
            return [];
        }

        $d = new \block_gamification_rule_property(\block_gamification_rule_base::EQ, 'd', 'crud');
        $c = new \block_gamification_rule_property(\block_gamification_rule_base::EQ, 'c', 'crud');
        $r = new \block_gamification_rule_property(\block_gamification_rule_base::EQ, 'r', 'crud');
        $u = new \block_gamification_rule_property(\block_gamification_rule_base::EQ, 'u', 'crud');

        // Skip those as they duplicate other more low level actions.
        $bcmv = new \block_gamification_rule_event('\mod_book\event\course_module_viewed');
        $dsc = new \block_gamification_rule_event('\mod_forum\event\discussion_subscription_created');
        $sc = new \block_gamification_rule_event('\mod_forum\event\subscription_created');
        $as = new \block_gamification_rule_property(\block_gamification_rule_base::CT, 'assessable_submitted', 'eventname');
        $au = new \block_gamification_rule_property(\block_gamification_rule_base::CT, 'assessable_uploaded', 'eventname');

        $list = array();

        $ruleset = new \block_gamification_ruleset(array($bcmv, $dsc, $sc, $as, $au), \block_gamification_ruleset::ANY);
        $data = array('rule' => $ruleset, 'points' => 0);
        $list[] = \block_gamification_filter::load_from_data($data);
     ...
     * @return block_gamification_filter[]
     */
    public function get_filters($category = \block_gamification_filter::CATEGORY_EVENTS) {
        // if (!$this->is_customised()) {
        //     // Early bail, saving one query.
        //     return $this->get_default_filters($category);
        // } //comment by revathi

        $results = $this->db->get_recordset('block_gamification_filters', ['courseid' => 0, 'category' => $category],
    ...
    public function is_customised() {
        return get_config('block_gamification', self::CUSTOMISED_CONFIG_KEY);
    }
    ...
    public function mark_as_customised() {
        set_config(self::CUSTOMISED_CONFIG_KEY, 1, 'block_gamification');
    }
    ...
    public function reset() {
        $this->db->delete_records('block_gamification_filters', ['courseid' => 0]);
        unset_config(self::CUSTOMISED_CONFIG_KEY, 'block_gamification');
    }
    ...
    public function reset_all_courses_to_defaults() {
        ...
        $courseids = $this->db->get_fieldset_select('block_gamification_config', 'courseid', $sql, [
