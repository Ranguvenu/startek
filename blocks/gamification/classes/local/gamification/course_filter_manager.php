...
class course_filter_manager {
    ...
    public function __construct(moodle_database $db, $courseid) {
        ...
        $this->cache = cache::make('block_gamification', 'filters');
    }
     ...
     * Get the filters.
     *
     * @param int $category The filter category, see block_gamification_filter::CATEGORY_* constants.
    ...
    public function get_static_filters() {
        ...
        $ruleset = new \block_gamification_ruleset([$bcmv, $dsc, $sc, $as, $au], \block_gamification_ruleset::ANY);
     ...
     */
    public function get_user_filters($category = \block_gamification_filter::CATEGORY_EVENTS) {
        $results = $this->db->get_recordset('block_gamification_filters', ['courseid' => $this->courseid,
    ...
    protected function import_filters(array $filters) {
        $sortorder = (int) $this->db->get_field('block_gamification_filters', 'COALESCE(MAX(sortorder), -1) + 1',
        ...
        foreach ($filters as $filter) {
            ...
            $newfilter = \block_gamification_filter::load_from_data($record);
            $newfilter->save();
        }
     ...
     * @return void
     */
    public function invalidate_filters_cache($category = \block_gamification_filter::CATEGORY_EVENTS) {
    ...
    public function purge() {
        $this->db->delete_records('block_gamification_filters', ['courseid' => $this->courseid]);
