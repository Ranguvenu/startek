...
namespace block_gamification\local\indicator;
...
class user_notice_indicator extends proxy_user_indicator implements user_indicator_with_acceptance {
    ...
    public function set_acceptable_user_flag($flag) {
        global $SESSION;
        if (!isset($SESSION->block_gamification_user_notice_indicator)) {
            $SESSION->block_gamification_user_notice_indicator = [];
        }
        $SESSION->block_gamification_user_notice_indicator[$flag] = true;
    }
    ...
    public function set_user_flag($userid, $flag, $value) {
        global $SESSION;
        if ($this->requiresflag && !isset($SESSION->block_gamification_user_notice_indicator[$flag])) {
            // Silent ignore.
            return;
        }
