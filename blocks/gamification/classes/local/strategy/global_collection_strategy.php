...
use block_gamification\local\factory\course_world_factory;
...
class global_collection_strategy implements event_collection_strategy {
    ...
    public function collect_event(\core\event\base $event) {
        ...
        if ($event->component === 'block_gamification') {
            // Skip own events.
            return;
        ...
        try {
            ...
            $canearn = has_capability('block/gamification:earngamification', $event->get_context(), $userid);
        } catch (moodle_exception $e) {
            return;
        }

        // Skip the events if the user does not have the capability to earn gamification.
        if (!$canearn) {
            return;
        }
