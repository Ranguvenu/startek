...
class course_world_collection_strategy implements event_collection_strategy {
    ...
    public function collect_event(\core\event\base $event) {
        ...
        // Get gamification to reward with.
        ...
        // Collect.
        // No need to go through the following if the user did not gain gamification.
        if ($points > 0) {
     ...
     *
     * @param \core\event\base $event The event.
     * @param \block_gamification\local\config\config $config The config.
     * @return bool True when the event is OK.
     */
    protected function can_capture_event(\core\event\base $event, \block_gamification\local\config\config $config) {
        global $SESSION;
        ...
        if (!isset($SESSION->block_gamification_cheatguard)) {
            // Init the session variable.
            $SESSION->block_gamification_cheatguard = [];
        } else {
            // Ensure that all entries are arrays, they may not be when we just upgraded the plugin.
            $SESSION->block_gamification_cheatguard = array_map(function($entry) {
                return is_array($entry) ? $entry : [$entry];
            }, $SESSION->block_gamification_cheatguard);
        }

        // Perform the check.
        if (!static::is_action_accepted($actionkey, $now, $SESSION->block_gamification_cheatguard, $maxactions, $maxtime, $actiontime)) {
            return false;
        }

        // Move the action at the end of the array.
        $times = isset($SESSION->block_gamification_cheatguard[$actionkey]) ? $SESSION->block_gamification_cheatguard[$actionkey] : [];
        unset($SESSION->block_gamification_cheatguard[$actionkey]);
        $SESSION->block_gamification_cheatguard[$actionkey] = $times;

        // Log the time at which this event happened.
        $SESSION->block_gamification_cheatguard[$actionkey][] = time();
        ...
        $SESSION->block_gamification_cheatguard = array_filter(array_map(function($times) use ($maxcount, $timethreshold) {
        ...
        $SESSION->block_gamification_cheatguard = array_slice($SESSION->block_gamification_cheatguard, -$maxcount, null, true);
    ...
    public static function is_action_accepted($action, $now, array $log, $maxactions, $maxintime, $timebetweenrepeats) {
        ...
        } else if ($timebetweenrepeats == 0) { // If timebetweenrepeats is 0
            if(isset($log[$action]) && sizeof($log[$action]) >= 1) {
                return false;
            }            
        }
