 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_gamification_rule_cm extends block_gamification_rule_property {
    ...
    protected function get_display_name() {
        if (empty($this->value)) {
            return get_string('errorunknownmodule', 'block_gamification');
        }
        ...
        return get_string($str, 'block_gamification', (object) $strparams);
    }
    ...
    protected function get_advanced_form($basename) {
        ...
        $o .= html_writer::start_tag('span', ['class' => 'block_gamification-cm-rule-widget ' . ($hascm ? 'has-cm' : null)]);
        ...
        if (!$hascm) {
            ...
            $o .= html_writer::tag('button', get_string('clicktoselectcm', 'block_gamification'), ['class' => 'btn btn-warning']);
        ...
        $o .= $output->help_icon('rulecm', 'block_gamification');
        $o .= html_writer::end_tag('span');
        return $o;
    }
    ...
    protected function get_simple_form($basename) {
        ...
        if (!$valuefound) {
            $options[] = [get_string('error') => [$this->value => get_string('errorunknownmodule', 'block_gamification')]];
        }

        $o = block_gamification_rule::get_form($basename);
        $modules = html_writer::select($options, $basename . '[value]', $this->value, '', array('id' => '', 'class' => ''));
        $helpicon = $output->help_icon('rulecm', 'block_gamification');

        $o .= html_writer::start_div('gamification-flex gamification-gap-1 gamification-min-full');
        $o .= html_writer::start_div('gamification-flex gamification-items-center');
        $o .= get_string('activityoresourceis', 'block_gamification', '');
        $o .= html_writer::end_div();
        $o .= html_writer::div($modules . $helpicon, 'gamification-flex gamification-items-center gamification-min-w-px gamification-whitespace-nowrap');
        $o .= html_writer::end_div();
    ...
    protected static function init_page_requirements() {
        ...
        $PAGE->requires->js_call_amd('block_gamification/cm-rule', 'init', $args);
        $PAGE->requires->strings_for_js(['cmselector', 'rulecmdescwithcourse'], 'block_gamification');
