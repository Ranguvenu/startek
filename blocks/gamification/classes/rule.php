 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class block_gamification_rule implements renderable {

    /**
     * Create a ruleset object from exported data.
     *
     * This method helps restoring a tree of rules without having to check
     * what the first rule is. Simply call block_gamification_rule::create($properties).
     *
     * This then calls the method {@see self::import()}.
     *
     * @param array $properties Array of properties acquired from {@see self::export()}.
     * @return block_gamification_rule|false The rule object.
     */
    public static function create(array $properties) {
        $classname = $properties['_class'];
        if (!class_exists($classname) || !is_subclass_of($classname, 'block_gamification_rule')) {
            return false;
        }
        $class = new $classname();
    ...
    protected function get_renderer() {
        return \block_gamification\di::get('renderer');
    }
