 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_gamification_ruleset extends block_gamification_rule {
     ...
     *
     * Those must implement the interface block_gamification_rule.
     *
     * @var string
     */
     ...
     *
     * @param block_gamification_rule $rule The rule to add.
     */
    public function add_rule(block_gamification_rule $rule) {
        $this->rules[] = $rule;
    }
    ...
    public function get_description() {
        return get_string('ruleset:' . $this->method, 'block_gamification');
    }
    ...
    public function get_form($basename) {
        ...
        $o .= html_writer::select(array(
           /* self::ALL => get_string('ruleset:all', 'block_gamification'),*/
            self::ANY => get_string('ruleset:any', 'block_gamification'),
            /*self::NONE => get_string('ruleset:none', 'block_gamification'),*/
        ), $basename . '[method]', $this->method, '', array('class' => '', 'id' => ''));
        return $o; //comment by revathi for ruleset all and none
    }
    ...
    protected function import(array $properties) {
        if (isset($properties['rules'])) {
            ...
            foreach ($properties['rules'] as $rule) {
                $ruleobject = block_gamification_rule::create($rule);
                if ($ruleobject) {
