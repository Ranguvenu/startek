...
$redirect = $CFG->wwwroot.'/local/costcenter/manageuserfields.php?category='.$categoryid;
...
// Show all categories.
$categories = $DB->get_records('user_info_category', ['id' => $categoryid]);
...
foreach ($categories as $category) {
    ...
    foreach ($options as $type => $fieldname) {       
        if(!in_array($fieldname,['Social','Checkbox','Drop-down menu']) ){
...
echo $OUTPUT->render_from_template('local_costcenter/edit_profile_fields', [
    'categories' => $outputcategories,
    'sesskey' => sesskey(),
    'baseurl' => (new moodle_url('/local/costcenter/manageuserfields.php'))->out(false)
]);
