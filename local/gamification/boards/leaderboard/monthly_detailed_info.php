...
$PAGE->set_url('/local/gamification/boards/leaderboard/monthly_detailed_info.php?course='.$courseid.'&eventname='.$eventname);
...
$PAGE->set_title(get_string('monthlydata', 'local_gamification'));
...
$PAGE->navbar->add(get_string('monthlydata', 'local_gamification'));

echo $OUTPUT->header();

echo "<h2 class='tmhead2'>".get_string('monthlydata', 'local_gamification').'</h2>';
if($courseid == SITEID) {
 ...
 $output .='<table class="leaderboard" id = "picture_list" cellpadding="30" cellspacing="10" style="width:100%;">
    ...
    $output .= html_writer::script("$(document).ready(function() {
					...
					var oTable = $('#picture_list').DataTable( {
						...
						'processing': true,
						'serverSide': true,
						'ajax': M.cfg.wwwroot + '/local/gamification/boards/leaderboard/monthly_ajax.php?eventname=$eventname&course=$courseid',
