...
$employee = ''; $level = 0;
    ...
    // if($levelinfo){
    //      $sql1 = "SELECT userid FROM {{$tablename}} WHERE month = (SELECT max(month) from {{$tablename}}) GROUP BY monthlylevel ORDER BY monthlylevel DESC ";
    // } else {
    //      $sql1 = "SELECT userid FROM {{$tablename}} WHERE month = (SELECT max(month) from {{$tablename}}) GROUP BY monthlyrank ORDER BY monthlyrank ASC ";
    // }                                            
    ...
    if($levelinfo == 'level'){                  
          $sql2 = 'SELECT u.id, u.firstname, u.lastname, u.email, x.monthlylevel as rank, x.monthlypoints as points ';//.user_picture::fields('u') . ', COALESCE(x.rank, 1) AS rank, x.gamification ' ;

          $sql2 .= " FROM {{$tablename}} x
                      JOIN {user} u
                      ON u.id = x.userid ";
                      $sql2 .= " WHERE x.month = (SELECT max(month) from {{$tablename}}) ";
                // $params = $inparams;
      // }
    } else {
        $sql2 = 'SELECT x.id as objectid, u.id, u.firstname, u.lastname, u.email, x.monthlyrank as rank, x.monthlypoints as points ';//.user_picture::fields('u') . ', COALESCE(x.rank, 1) AS rank, x.gamification ' ;
    ...
    {
        if($levelinfo == 'level'){
            $sql2.= " and x.monthlylevel = {$level} ";
        } else {
            $sql2.= " and x.monthlyrank = {$level} ";            
        }
    }
    ...
    if($levelinfo == 'level'){
        $sql2 .=  " ORDER BY x.monthlylevel DESC";
    } else {
        $sql2 .=  " ORDER BY x.monthlyrank ASC";          
    }
    ...
    foreach ($rec as $userid => $ups) {
        ...
        if($user)
            if($user->id == $USER->id || is_siteadmin()){
                ...
                $attributes = array('class'=>'points', 'onclick' => '(function(e){ require(\'gamificationboards_leaderboard/pointsinfo\').pointsInfo({selector:\'user_points_description_modal\', context:'.$systemcontext->id.',userid:'.$user->id.',eventname:"'.$eventname.'",type:"month",objectid:'.$ups->objectid.',points:'.$ups->points.',courseid:'.$courseid.'}) })(event)');
            }
            else{
        ...
        $points = '<span class="points_number">Points: </span>'.html_writer::tag('div', $ups->points, $attributes);
