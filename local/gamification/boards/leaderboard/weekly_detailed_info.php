...
global $DB, $OUTPUT,$USER,$CFG,$PAGE,$COURSE;
$eventname = required_param('eventname',PARAM_TEXT);
$courseid = optional_param('course', SITEID, PARAM_RAW);
// $lvl = optional_param('lvl', 0, PARAM_INT);

$PAGE->requires->jquery();
$PAGE->requires->jquery_plugin('ui');
$PAGE->requires->js('/local/gamification/boards/leaderboard/js/jquery.dataTables.min.js',true);
...
$PAGE->set_url('/local/gamification/boards/leaderboard/weekly_detailed_info.php?course='.$courseid.'eventname='.$eventname);                                 
...
$PAGE->set_title(get_string('weeklydata', 'local_gamification'));
...
$PAGE->navbar->add(get_string('weeklydata', 'local_gamification'));

echo $OUTPUT->header();

echo "<h2 class='tmhead2'>".get_string('weeklydata', 'local_gamification').'</h2>';
if($courseid == SITEID) {
...
echo html_writer::tag('div', $back, array('class'=>'viewall pull-right'));
   ...
   $output .='<div class="listng_container text-center w-100 pull-left">
                ...
                <div class="form-group col-md-6 col-sm-6 col-12 pull-left">
                	<label for="#1">'.get_string('rank', 'gamificationboards_leaderboard').'</label>
                 	<input type="text" id="1"  class="employee-search-input form-control" placeholder="Rank">
				</div>
				<div class="form-group col-md-6 col-sm-6 col-12 pull-left">
					<label for="#2">'.get_string('employee_name', 'gamificationboards_leaderboard').'</label>
 ...
 $output .='<table class="leaderboard" id = "picture_list" cellpadding="30" cellspacing="10" style="width:100%;">
    ...
    $output .= html_writer::script("$(document).ready(function() {
					...
					var oTable = $('#picture_list').DataTable( {
						...
						'processing': true,
						'serverSide': true,
						'ajax': M.cfg.wwwroot + '/local/gamification/boards/leaderboard/weekly_ajax.php?eventname=$eventname&course=$courseid',
