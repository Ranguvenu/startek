...
class expired_challenge_trigger extends \core\task\scheduled_task{
	public function get_name() {
        return get_string('task_expired_challenge_trigger', 'local_challenge');
    }
    public function execute(){
		...
		$expired_challenges = $DB->get_fieldset_sql($expired_challenges_sql, array('challenge_status' => CHALLENGE_NEW, 'currenttime' => time()));
		// $notification = new \local_challenge\notification();
		$challenge_lib = new \local_challenge\local\lib();
		foreach($expired_challenges AS $challenge){
			$challenge_lib->challenge_alter_status($challenge, CHALLENGE_EXPIRED);
			// $update_data = new \stdClass();
			// $update_data->id = $challenge->id;
			// $update_data->timemodified = time();
			// $update_data->status = CHALLENGE_EXPIRED;
