...
list($title, $subtitle) = \local_competency\page_helper::setup_for_course($url, $course, $subtitle);
...
$nav = new \local_competency\output\user_competency_course_navigation($userid, $competencyid, $courseid, $baseurl);
echo $output->render($nav);
if ($userid > 0) {
    $page = new \local_competency\output\user_competency_summary_in_course($userid, $competencyid, $courseid);
    ...
    echo $output->notify_problem(get_string('noparticipants', 'local_competency'));
}
