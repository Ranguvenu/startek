...
define(['jquery',
        ...
        'local_competency/advancedview_courselist',
        
        'local_competency/competencyruleconfig'],
       function($, url, templates, notification, str, ajax, dragdrop, Ariatree, Dialogue, menubar, Picker, Outcomes, advancedviewCourselist, RuleConfig
      ) {
    ...
    var reloadPage = function(context) {
        templates.render('local_competency/view_competencies_page', context)
    ...
    var updateSearchHandler = function(e) {
        ...
        var requests = ajax.call([{
            methodname: 'local_competency_data_for_view_competencies_page',
            args: {competencyframeworkid: frameworkid,
                    search: $('[data-region="filtercompetencies"] input').val()}
        }]);
            ...
            templates.render('local_competency/user_competency_brief', context).then(function(html,js) {
              //  alert('I ma here');
                ...
                $('[data-region="competencyinfo'+competency.competencyframeworkid+'"]').replaceWith(html);
                  $('[data-region="usercompetencybrief'+competency.competencyframeworkid+'"]').replaceWith(html);
    ...
    * @method deleteRelatedHandler
    */
    var AdvancedUserCompentencyView = function(competency, course) {
    ...
    var viewSelector=function(view, currentselector,treeModel){

           if(view=="basic")
                $(currentselector).on('selectionchanged', selectionChanged. bind(rememberCurrent));
            else if(view=="advanced"){
                //console.log(advancedviewCourselist);
                advancedviewCourselist._callcourseview(currentselector,treeModel);
               // $(currentselector).on('selectionchanged', selectionChanged. bind(rememberCurrent));
            }
            else
                $(currentselector).on('selectionchanged', selectionChanged. bind(rememberCurrent));     
    ...
    return {
         ...
         get_competencycourse_activities : function(competencyid, competencycourseid){                    
            advancedviewCourselist._callactivities(competencyid,competencycourseid);       
        },
        /**
         ...
         */
        init: function(model, pagectxid, taxonomies, rulesMods, currentselector, view) {
          ...
          $('[data-region="competencyactions"] [data-action="add"]').on('click', addHandler);
           ...
           viewSelector(view, currentselector, treeModel);
