 ...
 *
 * @module     local_competency/scaleconfig
 * @package    local_competency
 * @copyright  2015 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
define(['jquery', 'core/notification', 'core/templates', 'core/ajax', 'local_competency/dialogue', 'local_competency/scalevalues'],
    ...
    ScaleConfig.prototype.showConfig = function() {
        ...
        this.getScaleValues(this.scaleid).done(function() {
            ...
            // Dish up the form.
            templates.render('local_competency/scale_configuration_page', context)
                .done(function(html) {
                    new Dialogue(
                        scalename,
                        html,
            ...
            if (currentconfig !== '') {
                currentconfig.forEach(function(value) {
                    if (value.scaledefault === 1) {
                        body.find('[data-field="local_competency_scale_default_' + value.id + '"]').attr('checked', true);
                    }
                    if (value.proficient === 1) {
                        body.find('[data-field="local_competency_scale_proficient_' + value.id + '"]').attr('checked', true);
                    }
                });
            }
        }
    ...
    ScaleConfig.prototype.setScaleConfig = function() {
        ...
        this.scalevalues.forEach(function(value) {
            var scaledefault = 0;
            var proficient = 0;
            if (body.find('[data-field="local_competency_scale_default_' + value.id + '"]').is(':checked')) {
                scaledefault = 1;
            }
            if (body.find('[data-field="local_competency_scale_proficient_' + value.id + '"]').is(':checked')) {
                proficient = 1;
            }
