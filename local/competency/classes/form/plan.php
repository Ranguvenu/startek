...
class plan extends persistent {
    ...
    public function definition() {
        ...
        // Name.
        $mform->addElement('text', 'name', get_string('planname', 'local_competency'), 'maxlength="100"');
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required', null, 'client');
        $mform->addRule('name', get_string('maximumchars', '', 100), 'maxlength', 100, 'client');
        // Description.
        $mform->addElement('editor', 'description', get_string('plandescription', 'local_competency'), array('rows' => 4));
        $mform->setType('description', PARAM_CLEANHTML);

        $mform->addElement('date_time_selector', 'duedate', get_string('duedate', 'local_competency'), array('optional' => true));
        ...
        if ($plan->get('id')) {
            ...
            $mform->addElement('static', 'staticstatus', get_string('status', 'local_competency'), $plan->get_statusname());
        } else if (!empty($status) && count($status) > 1) {
            // There is more than one status to select from.
            $mform->addElement('select', 'status', get_string('status', 'local_competency'), $status);
        } else if (count($status) === 1) {
            // There is only one status to select from.
            $mform->addElement('static', 'staticstatus', get_string('status', 'local_competency'), current($status));
        } else {
