...
class competency extends persistent {
    ...
    public function definition() {
        ...
        $mform->addElement('static',
                           'frameworkdesc',
                           get_string('competencyframework', 'local_competency'),
                           s($framework->get('shortname')));

        $mform->addElement('hidden', 'parentid', '', array('id' => 'local_competency_parentcompetency'));

        $mform->setType('parentid', PARAM_INT);
        $mform->setConstant('parentid', ($parent) ? $parent->get('id') : 0);
        $parentlevel = ($parent) ? $parent->get_level() : 0;
        $parentname = ($parent) ? $parent->get('shortname') : get_string('competencyframeworkroot', 'local_competency');
        $parentlabel = '';
        if (!empty($competency->get('id'))) {
            $parentlabel = get_string('taxonomy_parent_' . $framework->get_taxonomy($parentlevel), 'local_competency');
        } else {
            $parentlabel = get_string('parentcompetency', 'local_competency');
        }
        $editaction = '';
        if (!$competency->get('id')) {
            $icon = $OUTPUT->pix_icon('t/editinline', get_string('parentcompetency_edit', 'local_competency'));
        ...
        if (!$competency->get('id')) {
            ...
            $PAGE->requires->js_call_amd('local_competency/parentcompetency_form', 'init', array('#id_parentcompetencybutton',
                '#local_competency_parentcompetency',
                '#id_parentdesc',
                $framework->get('id'),
                $pagecontextid));
        }
        ...
        $scales = array(null => get_string('inheritfromframework', 'local_competency')) + get_scales_menu();
