...
class page_helper {
    ...
    public static function setup_for_course(moodle_url $url, $course, $subtitle = '') {
        ...
        if (!empty($subtitle)) {
            ...
            $title = get_string('coursecompetencies', 'local_competency');
        }
        ...
        if (!empty($subtitle)) {
            $PAGE->navbar->add(get_string('coursecompetencies', 'local_competency'), $returnurl);
    ...
    public static function setup_for_template($pagecontextid, moodle_url $url, $template = null, $subtitle = '',
        ...
        if (!empty($template)) {
            ...
            $title = get_string('templates', 'local_competency');
        }
    ...
    public static function setup_for_plan($userid, moodle_url $url, $plan = null, $subtitle = '', $returntype = null) {
        ...
        if (!empty($plan)) {
            ...
            $title = get_string('learningplans', 'local_competency');
        }
    ...
    public static function setup_for_user_evidence($userid, moodle_url $url, $evidence = null, $subtitle = '', $returntype = null) {
        ...
        if (!empty($evidence)) {
            ...
            $title = get_string('userevidence', 'local_competency');
        }
    ...
    public static function setup_for_framework($id, $pagecontextid, $framework = null, $returntype = null) {
        ...
        if (empty($id)) {
            $pagetitle = get_string('competencyframeworks', 'local_competency');
            $pagesubtitle = get_string('addnewcompetencyframework', 'local_competency');

            $url->remove_params(array('id'));
            $PAGE->navbar->add($pagesubtitle, $url);
        } else {
            $pagetitle = $framework->get('shortname');
            $pagesubtitle = get_string('editcompetencyframework', 'local_competency');
    ...
    public static function setup_for_competency($pagecontextid, moodle_url $url, $framework, $competency = null, $parent = null) {
        ...
        if (empty($competency)) {
            ...
            $subtitle = get_string('taxonomy_add_' . $framework->get_taxonomy($level), 'local_competency');
            ...
            $subtitle = get_string('taxonomy_edit_' . $framework->get_taxonomy($competency->get_level()), 'local_competency');
        }
