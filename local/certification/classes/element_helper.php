...
class element_helper {
    ...
    public static function render_form_element_font($mform) {
        $mform->addElement('select', 'font', get_string('font','local_certification'), \local_certification\certification::get_fonts());
        $mform->setType('font', PARAM_TEXT);
        $mform->setDefault('font', 'times');
        $mform->addHelpButton('font', 'font','local_certification');
        $mform->addElement('select', 'fontsize', get_string('fontsize','local_certification'),
            \local_certification\certification::get_font_sizes());
        $mform->setType('fontsize', PARAM_INT);
        $mform->setDefault('fontsize', 12);
        $mform->addHelpButton('fontsize', 'fontsize','local_certification');
    }
    ...
    public static function render_form_element_colour($mform) {
        $mform->addElement('certification_colourpicker', 'colour', get_string('fontcolour','local_certification'));
        $mform->setType('colour', PARAM_RAW); // Need to validate that this is a valid colour.
        $mform->setDefault('colour', '#000000');
        $mform->addHelpButton('colour', 'fontcolour','local_certification');
    }
    ...
    public static function render_form_element_position($mform) {
        $mform->addElement('text', 'posx', get_string('posx','local_certification'), array('size' => 10));
        $mform->setType('posx', PARAM_INT);
        $mform->setDefault('posx', 0);
        $mform->addHelpButton('posx', 'posx','local_certification');
        $mform->addElement('text', 'posy', get_string('posy','local_certification'), array('size' => 10));
    ...
    public static function render_form_element_width($mform) {
        $mform->addElement('text', 'width', get_string('elementwidth','local_certification'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'elementwidth','local_certification');
        $refpointoptions = array();
        $refpointoptions[self::CERTIFICATION_REF_POINT_TOPLEFT] = get_string('topleft','local_certification');
        $refpointoptions[self::CERTIFICATION_REF_POINT_TOPCENTER] = get_string('topcenter','local_certification');
        $refpointoptions[self::CERTIFICATION_REF_POINT_TOPRIGHT] = get_string('topright','local_certification');
        $mform->addElement('select', 'refpoint', get_string('refpoint','local_certification'), $refpointoptions);
        $mform->setType('refpoint', PARAM_INT);
        $mform->setDefault('refpoint', self::CERTIFICATION_REF_POINT_TOPCENTER);
        $mform->addHelpButton('refpoint', 'refpoint','local_certification');
    }
    ...
    public static function validate_form_element_colour($data) {
        ...
        if (!self::validate_colour($data['colour'])) {
            $errors['colour'] = get_string('invalidcolour','local_certification');
        }
        return $errors;
    }
    ...
    public static function validate_form_element_position($data) {
        ...
        if ((!isset($data['posx'])) || (!is_numeric($data['posx'])) || ($data['posx'] < 0)) {
            $errors['posx'] = get_string('invalidposition', 'certification', 'X');
        }
        // Check if posy is not set, or not numeric or less than 0.
        if ((!isset($data['posy'])) || (!is_numeric($data['posy'])) || ($data['posy'] < 0)) {
            $errors['posy'] = get_string('invalidposition', 'certification', 'Y');
        }
    ...
    public static function validate_form_element_width($data) {
        ...
        if (isset($data['width']) && $data['width'] < 0) {
            $errors['width'] = get_string('invalidelementwidth','local_certification');
        }
    ...
    public static function get_element_sequence($pageid) {
        ...
        $sql = "SELECT MAX(sequence) as maxsequence
                  FROM {local_certification_elements}
                 WHERE pageid = :id";
    ...
    public static function get_certificationid($elementid) {
        ...
        $sql = "SELECT c.id
            ...
            INNER JOIN {local_certification_pages} cp
                    ON c.templateid = cp.templateid
            INNER JOIN {local_certification_elements} ce
                    ON cp.id = ce.pageid
    ...
    public static function get_available_element_types() {
        ...
        $elementdir = "$CFG->dirroot/local/certification/element";
        if (file_exists($elementdir)) {
            ...
            foreach ($elementfolders as $elementfolder) {
                ...
                // Get the class name.
                $classname = '\\certificationelement_' . $foldername . '\\element';
                 // print_object($classname);
                // Ensure the necessary class exists.
                if (class_exists($classname)) {
                    $component = "certificationelement_{$foldername}";
    ...
    public static function get_certificationname($elementid,$user=2) {
        global $DB, $SITE;
        ...
        if(!has_capability('local/certification:managecertification',$usercontext)){
            $sql.=" INNER JOIN {local_certification_users} cu
                    ON cu.certificationid = c.id";
        }
        $sql.=" WHERE ce.id = :elementid";
        /*echo $sql;
        print_object($elementid);exit;*/
        // Check if there is a course associated with this element.
        if ($certification = $DB->get_record_sql($sql, array('elementid' => $elementid))) {
