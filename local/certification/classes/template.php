...
/**
 * Class represents a certification template.
 *
...
class template {
    ...
    public function save($data) {
        ...
        $savedata->id = $this->id;
        $savedata->name = $DB->get_field('local_certification_templts','name',array('id'=>$this->id));
        $savedata->timelocalified = time();

        $DB->update_record('local_certification_templts', $savedata);
    }
    ...
    public function add_page() {
        ...
        $sql = "SELECT MAX(sequence) as maxpage
                  FROM {local_certification_pages} cp
        ...
        // Insert the page.
        return $DB->insert_record('local_certification_pages', $page);
    }
    ...
    public function save_page($data) {
        ...
        if ($pages = $DB->get_records('local_certification_pages', array('templateid' => $data->tid))) {
            ...
            foreach ($pages as $page) {
                ...
                $p->timelocalified = $time;
                // Update the page.
                $DB->update_record('local_certification_pages', $p);
            }
        }
    }
        ...
        if ($elements = $DB->get_records_sql($sql, array('templateid' => $this->id))) {
            foreach ($elements as $element) {
                ...
                if ($e = \local_certification\element_factory::get_element_instance($element)) {
                    ...
                    $DB->delete_records('local_certification_elements', array('id' => $element->id));
                }
            }
        }

        // Delete the pages.
        if (!$DB->delete_records('local_certification_pages', array('templateid' => $this->id))) {
            return false;
        }

        // Now, finally delete the actual template.
        if (!$DB->delete_records('local_certification_templts', array('id' => $this->id))) {
            return false;
        }
    ...
    public function delete_page($pageid) {
        ...
        // Get the page.
        $page = $DB->get_record('local_certification_pages', array('id' => $pageid), '*', MUST_EXIST);

        // Delete this page.
        $DB->delete_records('local_certification_pages', array('id' => $page->id));

        // The element may have some extra tasks it needs to complete to completely delete itself.
        if ($elements = $DB->get_records('local_certification_elements', array('pageid' => $page->id))) {
            foreach ($elements as $element) {
                // Get an instance of the element class.
                if ($e = \local_certification\element_factory::get_element_instance($element)) {
                    $e->delete();
        ...
        $sql = "UPDATE {local_certification_pages}
    ...
    public function delete_element($elementid) {
        ...
        if ($e = \local_certification\element_factory::get_element_instance($element)) {
            ...
            // The plugin files are missing, so just remove the entry from the DB.
            $DB->delete_records('local_certification_elements', array('id' => $elementid));
        }

        // Now we want to decrease the sequence numbers of the elements
        // that are greater than the element we deleted.
        $sql = "UPDATE {local_certification_elements}
                   SET sequence = sequence - 1
                 WHERE pageid = :pageid
                   AND sequence > :sequence";
     ...
     * @param int $userid the id of the user whose certification we want to view
    ...
    public function generate_pdf($preview = false, $userid = null, $return = false) {
        ...
        if ($pages = $DB->get_records('local_certification_pages', array('templateid' => $this->id), 'sequence ASC')) {
          
            // Create the pdf object.
            $pdf = new \pdf();

            // If the template belongs to a certification then we need to check what permissions we set for it.
            if ($protection = $DB->get_field('local_certification', 'protection', array('templateid' => $this->id))) {
                if (!empty($protection)) {
    ...
    public function copy_to_template($copytotemplateid) {
        ...
        // Get the pages for the template, there should always be at least one page for each template.
        if ($templatepages = $DB->get_records('local_certification_pages', array('templateid' => $this->id))) {
            ...
            foreach ($templatepages as $templatepage) {
                ...
                $page->id = $DB->insert_record('local_certification_pages', $page);
                // Now go through the elements we want to load.
                if ($templateelements = $DB->get_records('local_certification_elements', array('pageid' => $templatepage->id))) {
                    foreach ($templateelements as $templateelement) {
                        ...
                        $element->id = $DB->insert_record('local_certification_elements', $element);
    ...
    public function get_cm() {
        ...
        if ($context->contextlevel === CONTEXT_MODULE) {
            return get_courselocalule_from_id('certification', $context->instanceid, 0, false, MUST_EXIST);
        }
    ...
    public function require_manage() {
        require_capability('local/certification:manage', $this->get_context());
    }
    ...
    public static function create($templatename, $contextid) {
        ...
        $template->timelocalified = $template->timecreated;
        $template->id = $DB->insert_record('local_certification_templts', $template);

        return new \local_certification\template($template);
    }
}
