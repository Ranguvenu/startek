...
class certification_courses_deleted extends \core\event\base {
    /**
     * Init method.
     *
     * @return void
    ...
    public function get_description() {
        ...
        $courseid=$DB->get_field_sql("SELECT courseid FROM {local_certification_courses} where id=$this->objectid");
        if($courseid){
            $certification_courses_name=$DB->get_field_sql("SELECT fullname FROM {course} where id=$courseid");
            $certification_id=$DB->get_field_sql("SELECT certificationid FROM {local_certification_courses} where id=$this->objectid");
            ...
            $stringHelper->certification_courses_name=$this->certification_courses_name;
            $stringHelper->courseid=$this->courseid;
            $stringHelper->certification_name=$this->certification_name;
            $stringHelper->certification_id=$this->certification_id;
            return get_string("strcerficationidcourcedeleted",'local_certification',$stringHelper);
        }else{
            $stringHelper=new stdClass();
            $stringHelper->firstname=$firstname;
            $stringHelper->userid=$this->userid;
            $stringHelper->objectid=$this->objectid;
            return get_string("strcerficationcourcedeleted",'local_certification',$stringHelper);
        }
    }
    ...
    public static function get_name() {
        return get_string('eventcertificationcourses_deleted', 'local_certification');
    }
