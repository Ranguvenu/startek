...
class online_sessions_integration{
    ...
    public function online_sessions_type($session,$new_session,$type=1,$action){
        ...
        if(!empty($get_config)){
			...
			if($visible){
				 $return=$this->certification_module_integration($instance_type[1],$session,$new_session,$type);
			}
           

        }
    }
    /*For Certification if the Online session is a bigblueButton */
    public function certification_module_integration($instance_type,$session,$new_session,$type){
        global $DB,$CFG,$USER;
        ...
        if($instance_type!='webexactivity'){
            $online_module->name=$DB->get_field('local_certification','name',array('id'=>$session->certificationid));
        ...
        $courseid=$DB->get_field('local_certification','course',array('id'=>$session->certificationid));
        if($courseid==0){
           $courseid=1;
        }
        ...
        $online_module->batchid=$session->certificationid;
        ...
        if($instance_type=='bigbluebuttonbn'){
                ...
                if(!empty($session->certificationid))
                      $get_trainers=$DB->get_records('local_certification_sessions',array('id'=>$session->id));
	 ...
	 //print_object($online_module);
       $add_moduleinfo=add_moduleinfo($online_module, $courseid, $mform = null);
		...
		if($add_moduleinfo->id){ 
			...
			$DB->update_record('local_certification_sessions', $up_session);
		}
