...
abstract class element {
    ...
    public function __construct($element) {
        $showposxy = get_config('certification', 'showposxy');
    ...
    public function save_form_elements($data) {
        ...
        $element->timelocalified = time();

        // Check if we are updating, or inserting a new element.
        if (!empty($this->id)) { // Must be updating a record in the database.
            $element->id = $this->id;
            return $DB->update_record('local_certification_elements', $element);
        } else { // Must be adding a new one.
            $element->element = $data->element;
            $element->pageid = $data->pageid;
            $element->sequence = \local_certification\element_helper::get_element_sequence($element->pageid);
            $element->timecreated = time();
            return $DB->insert_record('local_certification_elements', $element, false);
        }
    }
    ...
    public function delete() {
        ...
        return $DB->delete_records('local_certification_elements', array('id' => $this->id));
    }
