...
class certification {
    ...
    public static function upload_files($draftitemid, $contextid, $filearea = 'image') {
        ...
        require_once($CFG->dirroot . '/lib/filelib.php');
         file_save_draft_area_files($draftitemid, $contextid, 'local_certification', 'image',$draftitemid);
        //file_save_draft_area_files($draftitemid, $contextid, 'local_certification', $filearea, 0);
    }
    ...
    public static function get_issues($certificationid, $groupmode, $cm, $limitfrom, $limitnum, $sort = '') {
        ...
        $allparams = $conditionsparams + array('certificationid' => $certificationid);

        // Return the issues.
        $ufields = \user_picture::fields('u');
        $sql = "SELECT $ufields, ci.id as issueid, ci.code, ci.timecreated
                  FROM {user} u
            INNER JOIN {local_certification_issues} ci
                    ON u.id = ci.userid
                 WHERE u.deleted = 0
    ...
    public static function get_conditional_issues_sql($cm, $groupmode) {
        ...
        // Get all users that can manage this certification to exclude them from the report.
        $certmanagers = array_keys(get_users_by_capability($context, 'local/certification:manage', 'u.id'));
    ...
    public static function generate_code() {
        ...
        while (!$uniquecodefound) {
            if (!$DB->record_exists('local_certification_issues', array('code' => $code))) {
    ...
    public static function certificationtypes(){
        return array(1 => get_string('certification', 'local_certification'),
                     2 => get_string('learningplan', 'local_certification'),
                     3 => get_string('certificate', 'local_certification')
                 );
    }
    /**
    ...
    public function manage_certification($certification) {
        ...
        try {
            if ($certification->id > 0) {
                ...
                $local_certification=$DB->get_record_sql("SELECT id,startdate,enddate,allow_multi_session,instituteid FROM {local_certification} where id = {$certification->id} ");
                ...
                if($certification->map_certificate == 1){
                    $certification->certificateid = $certification->certificateid;
                }else{
                    $certification->certificateid = null;
                }
                $DB->update_record('local_certification', $certification);
                $templateid = $DB->get_field('local_certification_templts','id',array('certificationid'=>$certification->id));
                $tempateobj = new \stdClass();
                $tempateobj->id = $templateid;
                $tempateobj->name = $certification->name;
                $tempateobj->certificationid = $certification->id;
                $tempateobj->timemodified = time();
                $DB->update_record('local_certification_templts', $tempateobj);
                $this->certification_set_events($certification);
                $params = array(
                    'context' => context_system::instance(),
                ...
                //     \local_tags_tag::set_item_tags('local_certification', 'certification', $certification->id, context_system::instance(), $certification->tags, 0, $certification->costcenter, $certification->department);
                // }
                ...
                if (has_capability('local/certification:managecertification', context_system::instance())){ 
                    ...
                    $certification->subdepartment = -1;
                      if (!is_siteadmin() && (has_capability('local/certification:manage_owndepartments', context_system::instance())
                ...
                $certification->certificationid=$certification->id = $DB->insert_record('local_certification', $certification);
                ...
                $event = \local_certification\event\certification_created::create($params);
                $event->add_record_snapshot('local_certification', $certification->id);
                $event->trigger();
                ...
                $newitemid = $DB->insert_record('local_certification_templts', $certification);
            ...
            if ($certification->id) {
                ...
                $sessions_count=$DB->count_records('local_certification_sessions', array('certificationid' => $certification->id));
    ...
    */
    function certification_set_events($certification) {
        ...
        $eventid = $DB->get_field('event', 'id',
               array('modulename' => '0', 'instance' => 0, 'plugin'=> 'local_certification', 'plugin_instance'=>$certification->id, 'eventtype' => 'open', 'local_eventtype' => 'open'));
       ...
       $eventid = $DB->get_field('event', 'id',
               array('modulename' => '0', 'instance' => 0, 'plugin'=> 'local_certification', 'plugin_instance'=>$certification->id, 'eventtype' => 'close', 'local_eventtype' => 'close'));
    ...
    public function manage_certification_sessions($session) {
        ...
        try {
            ...
            if ($session->id > 0) {
                ...
                $event = \local_certification\event\certification_sessions_created::create($params);
                $event->add_record_snapshot('local_certification', $session->certificationid);
                $event->trigger();
                if ($session->id) {
                    if($session->onlinesession ==1){
    ...
    */
    function session_set_events($session) {
        ...
        $eventid = $DB->get_field('event', 'id',
               array('modulename' => '0', 'instance' => 0, 'plugin'=> 'local_certification', 'plugin_instance'=>$session->certificationid, 'plugin_itemid'=>$session->id, 'eventtype' => 'open', 'local_eventtype' => 'session_open'));
        ...
        $eventid = $DB->get_field('event', 'id',
               array('modulename' => '0', 'instance' => 0, 'plugin'=> 'local_certification', 'plugin_instance'=>$session->certificationid, 'plugin_itemid'=>$session->id, 'eventtype' => 'close', 'local_eventtype' => 'session_close'));
    ...
    public function manage_certification_completions($completions) {
        ...
        try { 
            if ($completions->id > 0) {
                ...
                $DB->update_record('local_certificatn_completion', $completions);
                $params = array(
                    'context' => context_system::instance(),
                ...
                $event->add_record_snapshot('local_certification',$completions->certificationid);
                $event->trigger();
            } else {
    ...
    public function location_date($data) {
        ...
        try {
            $local_certification=$DB->get_record_sql("SELECT id,instituteid FROM {local_certification} where id = {$data->id}");
            ...
            $DB->update_record('local_certification', $location);
           // $params = array(
           //    'context' => context_system::instance(),
    ...
    public function add_update_design_certification($data) {
        ...
        try {
            ...
            if(isset($certification->templateid)&&$certification->templateid>0){
                $certification_pages=$DB->get_field('local_certification_pages','id',array('templateid'=>$certification->templateid));
                if($certification_pages){
                    $certification->id =  $certification_pages;
                    $certification->timemodified =  time();
                    $newitemid = $DB->update_record('local_certification_pages', $certification);
                }else{
                    $certification->timecreated =  time();
                    $newitemid = $DB->insert_record('local_certification_pages', $certification);
                }
            }
        } catch (dml_exception $ex) {
            print_error($ex);
    ...
    /**
     * [certification_status_action description]
     * @method certification_status_action
     * @param  [type]                  $certificationid     [description]
     * @param  [type]                  $certificationstatus [description]
     * @return [type]                                   [description]
     */
    public function certification_status_action($certificationid, $certificationstatus) {
        global $DB,$USER,$CFG;
        if(file_exists($CFG->dirroot . '/local/lib.php')){
           require_once($CFG->dirroot . '/local/lib.php');
        }
        switch($certificationstatus) {
            case CERTIFICATION_NEW:
                $this->update_certification_status($certificationid, CERTIFICATION_ACTIVE);
                $params = array(
                    'context' => context_system::instance(),
                    'objectid' => $certificationid
                );
            
                $event = \local_certification\event\certification_publish::create($params);
                $event->add_record_snapshot('local_certification', $certificationid);
                $event->trigger();
            ...
            case CERTIFICATION_ACTIVE:
                ...
                $this->update_certification_status($certificationid, CERTIFICATION_ACTIVE);
                // Trigger certification created event.
                $certification = $DB->get_record('local_certification', array('id'=>$certificationid));
                $this->certification_set_events($certification); // added by sreenivas
                $certificationusers = $DB->get_records_menu('local_certification_users', array('certificationid' => $certificationid), 'id', 'id, userid');
                $certificationcourses = $DB->get_records_menu('local_certification_courses', array('certificationid' => $certificationid), 'id', 'id, courseid');
                $type = 'certification_enrol';
                $dataobj = $certificationid;
                $fromuserid = $USER->id;
                $emaillogs = new \local_certification\notification();
                ...
                if(!empty($certificationcourses)){
                    ...
                    foreach($certificationcourses as $certificationcourse) {
                        foreach($certificationusers as $certificationuser) {
                            $this->manage_certification_course_enrolments($certificationcourse, $certificationuser,'employee','enrol');
                            if($i==0){
                                // $emaillogs = new certificatenotifications_emails();
                                // $email_logs = $emaillogs->certificate_emaillogs($type,$dataobj,$certificationuser,$fromuserid);
                                // $emaillogs = new \local_certification\notification();
                                $touser = \core_user::get_user($certificationuser);
                                $email_logs = $emaillogs->certification_notification($type, $touser, $USER, $certificationinstance);
                            }
                        }
                        $i++;
                    }
                }elseif(empty($certificationcourses)){
                    foreach($certificationusers as $certificationuser) {
                        // $emaillogs = new certificatenotifications_emails();
                        // $email_logs = $emaillogs->certificate_emaillogs($type,$dataobj,$certificationuser,$fromuserid);
                        $touser = \core_user::get_user($certificationuser);
                ...
                $certificationtrainers = $DB->get_records_menu('local_certification_trainers', array('certificationid' => $certificationid), 'id', 'id, trainerid');
                foreach($certificationtrainers as $certificationtrainer) {
                    // $emaillogs = new certificatenotifications_emails();
                    // $email_logs = $emaillogs->certificate_emaillogs($type,$dataobj,$certificationtrainer,$fromuserid);
                    $touser = \core_user::get_user($certificationtrainer);
                    $email_logs = $emaillogs->certification_notification($type, $touser, $USER, $certificationinstance);
                }
            break;
            case CERTIFICATION_CANCEL:
                ...
                $event = \local_certification\event\certification_cancel::create($params);
                $event->add_record_snapshot('local_certification', $certificationid);
                $event->trigger();
                $this->update_certification_status($certificationid, CERTIFICATION_CANCEL);
                $certificationusers = $DB->get_records_menu('local_certification_users', array('certificationid' => $certificationid), 'id', 'id, userid');
                $certificationcourses = $DB->get_records_menu('local_certification_courses', array('certificationid' => $certificationid), 'id', 'id, courseid');
                $type = 'certification_cancel';
                $dataobj = $certificationid;
                $fromuserid = $USER->id;
                $local_certification=$DB->get_record_sql("SELECT id,status FROM {local_certification} where id= {$certificationid}");
                $emaillogs = new \local_certification\notification();
                $certificationinstance = $DB->get_record('local_certification', array('id' => $dataobj));
                if(!empty($certificationcourses)){
                    $i=0;
                    foreach($certificationcourses as $certificationcourse) {
                    foreach($certificationusers as $certificationuser) {
                            //$this->manage_certification_course_enrolments($certificationcourse, $certificationuser,'employee','unenrol');
                            if($i==0 && $local_certification->status!=0){
                                // $emaillogs = new certificatenotifications_emails();
                                // $email_logs = $emaillogs->certificate_emaillogs($type,$dataobj,$certificationuser,$fromuserid);
                                $touser = \core_user::get_user($certificationuser);
                                $email_logs = $emaillogs->certification_notification($type, $touser, $USER, $certificationinstance);
            ...
            case CERTIFICATION_HOLD:
                ...
                $event = \local_certification\event\certification_hold::create($params);
                $event->add_record_snapshot('local_certification', $certificationid);
                $event->trigger();
                $this->update_certification_status($certificationid, CERTIFICATION_HOLD);
                $certificationusers = $DB->get_records_menu('local_certification_users', array('certificationid' => $certificationid), 'id', 'id, userid');
                $certificationcourses = $DB->get_records_menu('local_certification_courses', array('certificationid' => $certificationid), 'id', 'id, courseid');
                $type = 'certification_hold';
                $dataobj = $certificationid;
                $fromuserid = $USER->id;
                // $local_certification=$DB->get_record_sql("SELECT id,status FROM {local_certification} where id= $certificationid");
                $emaillogs = new \local_certification\notification();
                $local_certification=$certificationinstance = $DB->get_record('local_certification', array('id' => $dataobj));
                if(!empty($certificationcourses)){
                    $i=0;    
                    foreach($certificationcourses as $certificationcourse) {
                    foreach($certificationusers as $certificationuser) {
            ...
            case CERTIFICATION_COMPLETED:
                ...
                $this->certification_completions($certificationid);
                $this->update_certification_status($certificationid, CERTIFICATION_COMPLETED);
                $params = array(
                    'context' => context_system::instance(),
                    'objectid' => $certificationid
                );
            
                $event = \local_certification\event\certification_completed::create($params);
                $event->add_record_snapshot('local_certification', $certificationid);
                $event->trigger();
                $emaillogs = new \local_certification\notification();
                $certificationusers = $DB->get_records_menu('local_certification_users', array('certificationid' => $certificationid), 'id', 'id, userid');
                ...
                // foreach($certificationcourses as $certificationcourse) {
                foreach($certificationusers as $certificationuser) {
                        // $this->manage_certification_course_enrolments($certificationcourse, $certificationuser);
                      if($certificationinstance->status!=0){
                        // $emaillogs = new certificatenotifications_emails();
                        // $email_logs = $emaillogs->certificate_emaillogs($type,$dataobj,$certificationuser,$fromuserid);
                        $touser = \core_user::get_user($certificationuser);
                        $email_logs = $emaillogs->certification_notification($type, $touser, $USER, $certificationinstance);
    ...
    public function certification_completions($certificationid){
        ...
        if(!empty($certificationusers)) {
            foreach($certificationusers as $certificationuser){
                ...
                // $event->add_record_snapshot('local_certification', $certificationid);
                // $event->trigger();
                if($certificationuser->completion_status){
                    $params = array(
                        'context' => \context_system::instance(),
                        'objectid' => $certificationid,
                        'courseid' => 1,
                        'userid' => $certificationuser->userid,
                        'relateduserid' => $certificationuser->userid,
                    );
                    $event = \local_certification\event\certification_user_completed::create($params);
                    $event->add_record_snapshot('local_certification', $certificationid);
                    $event->trigger();
                }
            }
    ...
    /**
     * [certification_self_enrolment description]
     * @param  [type] $certificationid   [description]
     * @param  [type] $certificationuser [description]
     * @return [type]                [description]
     */
    public function certification_self_enrolment($certificationid,$certificationuser, $request=false){
                global $DB;
                $return=-2;
                 $certification_capacity_check=$this->certification_capacity_check($certificationid);
               
                if(!$certification_capacity_check){
                    $this->certification_add_assignusers($certificationid,array($certificationuser), $request);
    ...
    public function manage_certification_automatic_sessions($certificationid,$certificationstartdate,$certificationenddate) {
        global $DB;
            ...
            $first_time=date("H:i",$certificationstartdate);
