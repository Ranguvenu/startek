YUI.add('moodle-local_certification-rearrange', function (Y, NAME) {

/**
 * Rearrange elements in the custom certification
 *
 * @class Y.M.local_certification.rearrange
 * @constructor
 */
...
Y.extend(Rearrange, Y.Base, {
    ...
    /**
     * The certification page we are displaying.
     */
    page: [],

    /**
     * The custom certification elements to display.
     */
    elements: [],
    ...
    savepositions: function(e) {
        ...
        Y.io(M.cfg.wwwroot + '/local/certification/customcertajax.php', {
            method: 'POST',
            data: params,
            on: {
                ...
                success: function() {
                    ...
                    var pageinput = formnode.one('[name=ctid]');
                    if (pageinput) {
                        var pageid = pageinput.get('value');
                        window.location = baseurl + '?ctid=' + pageid;
                    } else {
...
Y.namespace('M.local_certification.rearrange').init = function(templateid, page, elements) {
