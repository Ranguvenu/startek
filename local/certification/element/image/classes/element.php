...
namespace certificationelement_image;
...
class element extends \local_certification\element {
    ...
    public function validate_form_elements($data, $files) {
        ...
        if ((!isset($data['width'])) || (!is_numeric($data['width'])) || ($data['width'] < 0)) {
            $errors['width'] = get_string('invalidwidth', 'certificationelement_image');
        }

        // Check if height is not set, or not numeric or less than 0.
        if ((!isset($data['height'])) || (!is_numeric($data['height'])) || ($data['height'] < 0)) {
            $errors['height'] = get_string('invalidheight', 'certificationelement_image');
        }

        // Validate the position.
        //if (get_config('certification', 'showposxy')) {
            $errors += \local_certification\element_helper::validate_form_element_position($data);
        //}
    ...
    public function save_unique_data($data) {
        ...
        //print_object($data);
            if ($data->certificationimage > 0){
                ...
                WHERE filename != '.' and itemid = :itemid
                ORDER BY id DESC "; //LIMIT 1
                $fileid=$DB->get_field_sql($sql, array('itemid' => $data->certificationimage), 0, 1);
    ...
    public function render_html() {
        ...
        // Get the image.
        $fs = get_file_storage();
        if ($file = $fs->get_file($imageinfo->contextid, 'local_certification', $imageinfo->filearea, $imageinfo->itemid,
                $imageinfo->filepath, $imageinfo->filename)) {
            $url = \moodle_url::make_pluginfile_url($file->get_contextid(), 'local_certification', 'image', $file->get_itemid(),
    ...
    public function definition_after_data($mform) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, 'local_certification', 'image', 0, $this->filemanageroptions);
    ...
    public function after_restore($restore) {
        ...
        // Perform the update.
        $DB->set_field('local_certification_elements', 'data', $elementinfo, array('id' => $this->get_id()));
    }
    ...
    public function get_file() {
        ...
        return $fs->get_file($imageinfo->contextid, 'local_certification', $imageinfo->filearea, $imageinfo->itemid,
            $imageinfo->filepath, $imageinfo->filename);
    }
    ...
    public static function get_images() {
        ...
        if ($files = $fs->get_area_files(\context_system::instance()->id, 'local_certification', 'image', false, 'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = $file->get_filename();
            }
        }
        // Loop through the files uploaded in the course context.
        // if ($files = $fs->get_area_files(\context_course::instance($COURSE->id)->id, 'local_certification', 'image', false,
        //     'filename', false)) {
        //     foreach ($files as $hash => $file) {
        //         $arrfiles[$file->get_id()] = $file->get_filename();
        //     }
        ...
        $arrfiles = array('0' => get_string('noimage', 'local_certification')) + $arrfiles;
