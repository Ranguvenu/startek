...
namespace certificationelement_digitalsignature;

defined('MOODLE_INTERNAL') || die();

/**
 * The certification element digital signature's core interaction API.
 *
 * @package    certificationelement_digitalsignature
 * @copyright  2017 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class element extends \certificationelement_image\element {

    /**
     * @var array The file manager options for the certification.
     */
    ...
    public function render_form_elements($mform) {
        $mform->addElement('select', 'fileid', get_string('image', 'certificationelement_image'), self::get_images());

        $mform->addElement('select', 'signaturefileid', get_string('digitalsignature', 'certificationelement_digitalsignature'),
            self::get_signatures());

        $mform->addElement('text', 'signaturename', get_string('signaturename', 'certificationelement_digitalsignature'));
        $mform->setType('signaturename', PARAM_TEXT);
        $mform->setDefault('signaturename', '');

        $mform->addElement('passwordunmask', 'signaturepassword',
            get_string('signaturepassword', 'certificationelement_digitalsignature'));
        $mform->setType('signaturepassword', PARAM_TEXT);
        $mform->setDefault('signaturepassword', '');

        $mform->addElement('text', 'signaturelocation', get_string('signaturelocation', 'certificationelement_digitalsignature'));
        $mform->setType('signaturelocation', PARAM_TEXT);
        $mform->setDefault('signaturelocation', '');

        $mform->addElement('text', 'signaturereason', get_string('signaturereason', 'certificationelement_digitalsignature'));
        $mform->setType('signaturereason', PARAM_TEXT);
        $mform->setDefault('signaturereason', '');

        $mform->addElement('text', 'signaturecontactinfo',
            get_string('signaturecontactinfo', 'certificationelement_digitalsignature'));
        $mform->setType('signaturecontactinfo', PARAM_TEXT);
        $mform->setDefault('signaturecontactinfo', '');

        $mform->addElement('text', 'width', get_string('width', 'certificationelement_image'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'width', 'certificationelement_image');

        $mform->addElement('text', 'height', get_string('height', 'certificationelement_image'), array('size' => 10));
        $mform->setType('height', PARAM_INT);
        $mform->setDefault('height', 0);
        $mform->addHelpButton('height', 'height', 'certificationelement_image');
        ...
        $mform->addElement('filemanager', 'digitalsignature',
            get_string('uploaddigitalsignature', 'certificationelement_digitalsignature'),array('class'=>'hide_digitalsignature'),
            $this->signaturefilemanageroptions);
    }
    ...
    public function save_form_elements($data) {
        ...
        // Handle file uploads.
        \local_certification\certification::upload_files($data->certificationimage, $context->id);

        // Handle file certification uploads.
        \local_certification\certification::upload_files($data->digitalsignature, $context->id, 'signature');
    ...
    public function definition_after_data($mform) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, 'local_certification', 'signature', 0,
            $this->signaturefilemanageroptions);
    ...
    public static function get_signatures() {
        ...
        if ($files = $fs->get_area_files(\context_system::instance()->id, 'local_certification', 'signature', false,
                'filename', false)) {
        ...
        if ($files = $fs->get_area_files(\context_course::instance($COURSE->id)->id, 'local_certification', 'signature', false,
                'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = $file->get_filename();
            }
        ...
        $arrfiles = array('0' => get_string('nosignature', 'certificationelement_digitalsignature')) + $arrfiles;
    ...
    public function get_signature_file() {
        ...
        return $fs->get_file($imageinfo->signaturecontextid, 'local_certification', $imageinfo->signaturefilearea,
