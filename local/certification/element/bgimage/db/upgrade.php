...
/**
 * Certification background image element upgrade code.
 *
 * @param int $oldversion the version we are upgrading from
 * @return bool always true
 */
function xmldb_certificationelement_bgimage_upgrade($oldversion) {
    global $DB;

    if ($oldversion < 2016120501) {
        // Go through each 'image' element and update the file stored information.
        if ($images = $DB->get_records_select('local_certification_elements', $DB->sql_compare_text('element') . ' = \'bgimage\'')) {
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2016120501, 'certificationelement', 'bgimage');
    }
