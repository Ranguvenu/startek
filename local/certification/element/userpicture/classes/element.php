...
namespace certificationelement_userpicture;
 ...
 *
 * @package    certificationelement_userpicture
...
class element extends \local_certification\element {
    ...
    public function render_form_elements($mform) {
        $mform->addElement('text', 'width', get_string('width', 'certificationelement_userpicture'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'width', 'certificationelement_userpicture');

        $mform->addElement('text', 'height', get_string('height', 'certificationelement_userpicture'), array('size' => 10));
        $mform->setType('height', PARAM_INT);
        $mform->setDefault('height', 0);
        $mform->addHelpButton('height', 'height', 'certificationelement_userpicture');
    ...
    public function validate_form_elements($data, $files) {
        ...
        if ((!isset($data['width'])) || (!is_numeric($data['width'])) || ($data['width'] < 0)) {
            $errors['width'] = get_string('invalidwidth', 'certificationelement_userpicture');
        }

        // Check if height is not set, or not numeric or less than 0.
        if ((!isset($data['height'])) || (!is_numeric($data['height'])) || ($data['height'] < 0)) {
            $errors['height'] = get_string('invalidheight', 'certificationelement_userpicture');
        }
