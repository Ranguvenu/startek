...
if ($cspreport) {
    $documenturi = clean_param(remove_sesskey($cspreport['document-uri']), PARAM_URL);
    $blockeduri = clean_param(remove_sesskey($cspreport['blocked-uri']), PARAM_URL);
    $violateddirective = clean_param($cspreport['violated-directive'], PARAM_TEXT);
    ...
    $hash = sha1($documenturi . $blockeduri . $violateddirective);
