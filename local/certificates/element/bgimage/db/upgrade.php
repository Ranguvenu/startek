...
/**
 * Certificates background image element upgrade code.
 *
...
function xmldb_certificateelement_bgimage_upgrade($oldversion) {
    ...
    if ($oldversion < 2016120501) {
        ...
        if ($images = $DB->get_records_select('local_certificate_elements', $DB->sql_compare_text('element') . ' = \'bgimage\'')) {
            ...
            foreach ($images as $image) {
                ...
                if ($file = $fs->get_file_by_hash($elementinfo->pathnamehash)) {
                    ...
                    $DB->set_field('local_certificate_elements', 'data', $arrtostore,  array('id' => $image->id));
                }
            }
        }
