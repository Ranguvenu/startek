...
class element extends \local_certificates\element {
    ...
    public function render_form_elements($mform) {
      ...
      $dateoptions = array();
      // $dateoptions[CERTIFICATION_DATE_ISSUE] = get_string('issueddate', 'certificateelement_date');
      $dateoptions[CERTIFICATION_DATE_COMPLETION] = get_string('completiondate', 'certificateelement_date');
      // $dateoptions[CERTIFICATION_DATE_COURSE_START] = get_string('certificationstartdate', 'certificateelement_date');
      // $dateoptions[CERTIFICATION_DATE_COURSE_END] = get_string('certificationenddate', 'certificateelement_date');
    ...
    public function render($pdf, $preview, $user, $moduleinfo = false) {
      ...
      // $certificationid = \local_certificates\element_helper::get_certificationid($this->id);
      ...
      if ($preview) {
        ...
        $certification = $DB->get_record('local_certificate', array('id' => $page->certificateid), '*', MUST_EXIST);
        // Now we can get the issue for this user.
        $issue = $DB->get_record('local_certificate_issues', array('userid' => $user->id, 'certificateid' => $certification->id));

        if ($dateitem == CERTIFICATION_DATE_COMPLETION) {
        ...
        if($moduleinfo){
          switch($moduleinfo->moduletype) {
            case 'course':
              $completiondate = $DB->get_field('course_completions','timecompleted',array('course'=>$moduleinfo->moduleid,'userid'=>$user->id));
              break;
            case 'classroom':
              $completiondate = $DB->get_field('local_classroom_users','completiondate',array('classroomid'=>$moduleinfo->moduleid,'userid'=>$user->id));
              break;
            case 'learningplan':
              $completiondate = $DB->get_field('local_learningplan_user','completiondate',array('planid'=>$moduleinfo->moduleid,'userid'=>$user->id));
              break;
            case 'program':
              $completiondate = $DB->get_field('local_program_users','completiondate',array('programid'=>$moduleinfo->moduleid,'userid'=>$user->id));
                break;
            case 'onlinetest':
              $completiondate = $DB->get_field('local_onlinetest_users','timemodified',array('onlinetestid'=>$moduleinfo->moduleid,'userid'=>$user->id));
              break;
            default:
              $completiondate = time();
              break;
        ...
        //               FROM {local_certificate_users} c
        //              WHERE c.userid = :userid
        //                AND c.certificateid = :certificationid AND completion_status=:completionstatus";
        ...
        //     $date = $DB->get_field('local_certificate', 'startdate', array('id' => $certificationid));
        // } else if ($dateitem == CERTIFICATION_DATE_COURSE_END) {
        //     $date = $DB->get_field('local_certificate', 'enddate', array('id' => $certificationid));
        // }
        //  else {
        //     if ($dateitem == CERTIFICATION_DATE_COURSE_GRADE) {
        //         $grade = \local_certificate\element_helper::get_course_grade_info(
        //             $certificationid,
        ...
        //         $grade = \local_certificate\element_helper::get_grade_item_info(
        //             $gradeitemid,
        //             $dateitem,
        //             $user->id
        //         );
        //     } else {
        //         $grade = \local_certificate\element_helper::get_local_grade_info(
        //             $dateitem,
        //             GRADE_DISPLAY_TYPE_DEFAULT,
        //             $user->id
        //         );
      ...
      if (!empty($completiondate)) {
        \local_certificates\element_helper::render_content($pdf, $this, $this->get_date_format_string($completiondate, $dateformat));
      }
    }
    ...
    */
    public function after_restore($restore) {
      ...
      if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_localule', $dateinfo->dateitem)) {
          ...
          $DB->set_field('local_certificate_elements', 'data', $this->save_unique_data($dateinfo), array('id' => $this->get_id()));
      }
    }
