...
class querylib {
        ...
        $sql = "FROM {role_capabilities} as rc
                 ...
                 WHERE u.confirmed = :confirmed
                  AND u.suspended = :suspended AND u.deleted = :deleted AND u.id > 2
                       ...
                       AND rc.capability LIKE '%trainer_viewcertification%' and rc.permission=1 ";
        if (!empty($trainers)) {
    ...
    public function get_department_institute_list($costcenters, $institutetype) {
        ...
        $sql = "SELECT lci.id,CONCAT(lci.fullname, '-', lci.shortnname) AS fullshrtname
                ...
                WHERE lci.costcenter IN (:costcenterid) AND lci.institute_type = :institutetype 
                AND lci.visible= :visiblefld ";
    ...
    public function get_certification_institutes($institutetype = 0, $service = array()) {
        global $DB;
        $institutes = array();
        if ($institutetype > 0) {
            $params = array();
            ...
            if (!empty($service)) {
                ...
                if ($service['certificationid'] > 0) {
                    $institutessql .= " AND costcenter = :costcenter ";
                    $params['costcenter'] = $DB->get_field('local_certification', 'costcenter', array('id' => $service['certificationid']));
                }
    ...
    public function get_certification_institute_rooms($clasroomid) {
        global $DB;
        ...
        if ($clasroomid > 0) {
            $locationroomlistssql = "SELECT cr.id, cr.name
                ...
                JOIN {local_certification} AS c ON ( c.instituteid = ci.id
                AND c.institute_type = ci.institute_type)
                WHERE cr.visible = 1 AND ci.visible = 1 AND c.id = {$clasroomid} ";
