...
abstract class element {
    ...
    /**
     * @var obj $moduleinfo for get the particular element 
     */
    ...
    public function __construct($element) {
        $showposxy = get_config('certificates', 'showposxy');
    ...
    public function save_form_elements($data) {
        ...
        $element->usercreated = $USER->id;
        $element->usermodified = $USER->id;
        $element->timecreated = time();
        $element->timemodified = time();
        // Check if we are updating, or inserting a new element.
        if (!empty($this->id)) { // Must be updating a record in the database.
            $element->id = $this->id;
            return $DB->update_record('local_certificate_elements', $element);
        } else { // Must be adding a new one.
            $element->element = $data->element;
            $element->pageid = $data->pageid;
            $element->sequence = \local_certificates\element_helper::get_element_sequence($element->pageid);
            return $DB->insert_record('local_certificate_elements', $element, false);
        }
    }
    ...
    public function delete() {
        ...
        return $DB->delete_records('local_certificate_elements', array('id' => $this->id));
    }

    /**
     * This function is responsible for handling the restoration process of the element.
     *
     * For example, the function may save data that is related to another course localule, this
     * data will need to be updated if we are restoring the course as the course localule id will
