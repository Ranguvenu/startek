...
class page_helper {
     ...
     */
    public static function page_setup($pageurl, $context, $title, $includejs = false) {
    ...
    protected static function include_js() {
        global $PAGE;
        $PAGE->requires->jquery();
        $PAGE->requires->css('/local/ilp/css/jquery.dataTables.css');
        $PAGE->requires->js('/local/ilp/js/jquery.dataTables.min.js', true);
        $PAGE->requires->js('/local/ilp/js/ajax.js');
        $PAGE->requires->js('/local/ilp/js/custom_form.js');
    }
}
